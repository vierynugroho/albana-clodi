...
export async function getProvinces(): Promise<ResponseSuccess> {
	try {
...
export async function getCities(provinceId: string): Promise<ResponseSuccess> {
	try {
...
export async function getDistricts(cityId: string): Promise<ResponseSuccess> {
	try {
...
export async function getVillages(districtId: string): Promise<ResponseSuccess> {
	try {
		const { data } = await axios.get(`${apiUrl}/regions/villages/${districtId}`, {
			headers: {
				Authorization: `Bearer ${token}`,
			},
		});
