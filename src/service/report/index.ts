...
export async function getPaymentTransaction(query?: ExpenseQuery): Promise<ResponsePaymentTransaction> {
	try {
		const { data } = await axios.get(`${apiUrl}/reports/payments-transactions`, {
			params: query,
			headers: {
				'ngrok-skip-browser-warning': 'true',
				Authorization: `Bearer ${localStorage.getItem('token')}`,
...
export async function ProductSolds(query?: ExpenseQuery): Promise<ResponseProdcutSold> {
	try {
