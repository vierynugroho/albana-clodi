...
export const fetchCustomers = async (query: string): Promise<{ label: string; value: string; customer: TCustomer }[]> => {
	try {
		const response = await axios.get<GetCustomersResponse>(`${apiUrl}/customers`, {
			params: { search: query },
			headers: {
				Authorization: `Bearer ${token}`,
			},
...
export const fetchDeliveryPlace = async (query: string): Promise<{ label: string; value: string; place: TDeliveryPlace }[]> => {
	try {
		const response = await axios.get<GetResponseDeliveryPlace>(`${apiUrl}/delivery-places`, {
			params: { search: query },
			headers: {
				Authorization: `Bearer ${token}`,
			},
...
export const fetchProduct = async (query: string): Promise<{ label: string; value: string; product: ProductItem }[]> => {
	try {
...
export const fetchSalesChannels = async (query: string): Promise<{ label: string; value: string; channel: SalesChannel }[]> => {
	try {
		const response = await axios.get<SalesChannelResponse>(`${apiUrl}/sales-channels`, {
			params: { search: query },
			headers: {
				Authorization: `Bearer ${token}`,
			},
...
export const fetchPayments = async (query: string): Promise<{ label: string; value: string; payment: PaymentMethod }[]> => {
	try {
...
export const calculateShippingCost = async (params: ShippingCostParams): Promise<ShippingCostResponse> => {
	try {
		const response = await axios.get<ShippingCostResponse>(`${apiUrl}/shipping-cost/calculate`, {
			params,
			headers: {
				Authorization: `Bearer ${token}`,
			},
