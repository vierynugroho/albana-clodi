...
export default function FormCustomer() {
	...
	function handleChange<K extends keyof FormCustomer>(key: K, value: FormCustomer[K]) {
	...
	const handleSubmit = () => {
		const payload = {
			...
			address: `${formData.villageName}, ${formData.districtName}, ${formData.cityName}`,
				...
				>
					<div className='space-y-6 flex flex-wrap gap-4'>
						...
						<div className='flex-1/3'>
							...
							<SearchableDropdown
								options={provinces.map((p) => ({ id: Number(p.id), name: p.name }))}
								label='name'
								id='id'
						...
						<div className='flex-1/3'>
							...
							<SearchableDropdown
								options={cities.map((c) => ({ id: Number(c.id), name: c.name }))}
								label='name'
								id='id'
						...
						<div className='flex-1/3'>
							...
							<SearchableDropdown
								options={districts.map((d) => ({ id: Number(d.id), name: d.name }))}
								label='name'
								id='id'
								selectedVal={formData.districtName}
								handleChange={(val) => {
									const selectedDistrict = districts.find((d) => d.name === val);
									setFormData((prev) => ({
										...prev,
						...
						<div className='flex-1/3'>
							...
							<SearchableDropdown
								options={villages.map((v) => ({ id: Number(v.id), name: v.name }))}
								label='name'
								id='id'
