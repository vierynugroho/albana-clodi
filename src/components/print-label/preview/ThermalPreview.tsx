    ...
    return (
        <div className="font-mono px-1 w-64 py-5 text-[10px] space-y-3">
            <div className="px-4 flex flex-col justify-center items-center text-center space-y-2">
                {has("Logo") && <img src={Logo} alt="" className="w-2/3" />}
            ...
            {data?.products?.flatMap((product: TProduct, i) => {
                ...
                const discount = data?.discount?.persen || 0;

                return product.product_variants.map((variant, j) => {
                    const priceData = variant.productPrices?.[0] || {};

                    const originalPrice =
                        priceType === "reseller" ? priceData.reseller :
                            priceType === "agent" ? priceData.agent :
                                priceType === "customer" || priceType === "dropshipper" ? priceData.normal :
                                    priceData.normal || 0;

                    const discountedPrice = originalPrice - (originalPrice * discount / 100);
                    const qty = 1; // diasumsikan 1 per variant
                    const subtotal = discountedPrice * qty;

                    return (
                        <div key={`${i}-${j}`} className="space-y-1">
                            <div className="grid grid-cols-12 items-end">
                                <span className="col-span-5">{product.product_name} | {variant.sku}</span>
                                <span>{qty}</span>
