...
import ModalFormDiscount, { DiscountValueType, ItemType } from '../modal/ModalDiskon';
...
export default function TableAddOrder({ shipperDestinationId, receiverDestinationId, onChange, initialData, customerCategory }: TableAddOrderProps) {
	...
	const handleApplyDiscount = (productId: string, finalPrice: number, discount: number, discountType: 'Rp' | '%') => {
		setOrders((prev) =>
			prev.map((order) =>
				order.productId === productId
					? {
	...
	const totalFinal = useMemo(() => {
		...
		return totalSubtotal + otherAdditions + selectedShippingCost - discountAmount;
	...
	useEffect(() => {
		...
		let discountOrder: { value: number; type: 'nominal' | 'percent' } | null = null;
		...
		const productDiscount = orders
			.filter((order) => order.discount !== undefined && order.discountType !== undefined)
			.map((order) => ({
	...
	return (
		<>
				...
				<div className='overflow-hidden rounded-lg bg-white dark:border-white/[0.05] dark:bg-transparent'>
							...
							<TableBody className='divide-y divide-gray-100 dark:divide-white/[0.05]'>
								{orders.length > 0 ? (
									orders.map((order, index) => (
										<TableRow key={index}>
											<TableCell className='px-4 py-3 text-black text-start text-sm md:text-theme-md dark:text-gray-400'>{order.name}</TableCell>
											<TableCell className='px-4 py-3 text-black text-start text-sm md:text-theme-md dark:text-gray-400'>
												{formatPrice(order.harga)}
												{order.discount !== undefined && <div className='text-red-500 italic text-[10px]'>disc. {order.discountType === '%' ? `${order.discount}%` : formatPrice(order.discount)}</div>}
											</TableCell>
											<TableCell className='px-4 py-3 text-black text-start text-sm md:text-theme-md dark:text-gray-400'>{order.qty}</TableCell>
											<TableCell className='px-4 py-3 text-black text-start text-sm md:text-theme-md dark:text-gray-400'>{formatPrice(order.subtotal)}</TableCell>
										...
										>
											<div className='flex flex-col items-center justify-center'>
												<img
													...
													alt='box-empty'
												/>
												<span className='text-gray-300'>Belum ada produk ditambahkan</span>
											</div>
										</td>
									</TableRow>
								)}
					...
					<div className='border-t mt-6 pt-4 space-y-2'>
						...
						<div className='flex justify-between items-center'>
							...
							<span className='font-bold'>{formatPrice(selectedShippingCost)}</span>
						</div>
						...
						<div>
							{items.map((item, index) => (
								...
								>
									<div>
										{item.title}
										{item.name !== 'Ongkir' && item.name !== '' && <> - {item.name}</>}
										{item.discountType === '%' && <span className={item.title === 'Asuransi' || item.title === 'Biaya Lain' ? 'text-black' : 'text-red-500'}> ( {item.value}% )</span>}
									</div>

									<div className='flex items-center gap-2'>
										<span className={item.title === 'Asuransi' || item.title === 'Biaya Lain' ? 'text-black' : 'text-red-500'}>
											{item.discountType === '%' ? <span>( {(totalSubtotal * Number(item.value)) / 100} )</span> : <span>( {item.value} )</span>}
										</span>
                ...
                >
                  <span className="text-lg font-bold">+</span> Biaya Lain
						...
						<div className='flex justify-between items-center font-semibold pt-4 border-t mt-4'>
							<span>TOTAL</span>
							<span className='text-blue-700 text-xl md:text-3xl font-bold'>{formatPrice(totalFinal)}</span>
						</div>
					</div>
				</div>
			</ComponentCard>
			{modalType && (
				<ModalFormDiscount
					title={modalType === 'diskon' ? 'Diskon Order' : modalType === 'biaya' ? 'Biaya Lain' : modalType === 'asuransi' ? 'Asuransi' : modalType === 'ongkir' ? 'Diskon Ongkir' : ''}
					namePlaceholder={`Masukkan Nama ${
						modalType === 'diskon' ? 'Diskon Order' : modalType === 'biaya' ? 'Biaya Lain' : modalType === 'asuransi' ? 'Asuransi' : modalType === 'ongkir' ? 'Diskon Ongkir' : ''
					}`}
					changeModal={() => {
						setModalType(null);
					...
					onSubmit={(data) => {
						if (editingIndex !== null) {
							setItems((prev) => prev.map((item, i) => (i === editingIndex ? { ...item, ...data, type: data.type as ItemType } : item)));
						} else {
							setItems((prev) => [
								...prev,
								{
