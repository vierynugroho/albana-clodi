...
export default function ProductDiscountModal({ productName, initialPrice, initialDiscountValue = 0, initialDiscountType = 'Rp', onClose, onSave }: ProductDiscountModalProps) {
					...
					<div>
						...
						<input
							...
							onChange={(e) => {
								...
								if (discountType === 'Rp') {
									setDiscountValue(initialPrice - numValue);
								} else {
									const percentDiscount = ((initialPrice - numValue) / initialPrice) * 100;
									setDiscountValue(Math.min(100, Math.round(percentDiscount * 100) / 100));
								}
							}}
							className='w-full border rounded px-4 py-2 bg-white text-gray-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500'
						/>
					</div>
				</div>
