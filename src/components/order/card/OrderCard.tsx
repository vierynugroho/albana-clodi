...
export default function OrderCard({ orders, selectedOrderIds, onToggleSelect, variant = 'default' }: Props) {
	...
	if (!Array.isArray(orders) || orders.length === 0) {
		return <p className='text-center text-gray-500'>Tidak ada order ditemukan.</p>;
	}
			...
			{orders.map((order) => (
					...
					<div className='flex justify-between items-start'>
						...
						<div className='flex items-center space-x-4'>
							{TrackingProgress.map((step, index) => (
								...
								>
									<div
										className={`p-2 rounded-full border-2 cursor-pointer ${step.status === 'completed' ? 'border-green-500' : 'border-gray-300'} hover:border-green-500 transition duration-300`}
										title={`Status pengiriman: ${step.status}`}
									>
										<span className={step.status === 'success' ? 'text-green-500' : 'text-gray-400'}>{step.icon}</span>
									</div>
									{index < TrackingProgress.length - 1 && <div className='w-10 h-1 bg-gray-300 mx-2'></div>}
								</div>
							))}
						</div>
					</div>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6'>
						...
						<div className='space-y-3'>
							<div>
								...
								<p className='text-xl font-semibold'>{order.OrdererCustomer?.name || 'Tidak diketahui'}</p>
								<p className='text-sm text-purple-600'>{order.OrdererCustomer?.category || '-'}</p>
							</div>
							<div>
								<p className='font-normal text-gray-500'>Dikirim kepada</p>
								<p className='text-xl font-semibold'>{order.DeliveryTargetCustomer?.name || 'Tidak diketahui'}</p>
								<p className='text-sm text-purple-600'>{order.DeliveryTargetCustomer?.category || '-'}</p>
							</div>
							<div>
								<p className='font-normal text-gray-500'>Catatan</p>
								<p className='text-md font-light text-gray-dark'>{order.note || '-'}</p>
							</div>
						</div>
						...
						<div className='space-y-2'>
							<div>
								...
								<div className='border dark:border-gray-500 rounded-lg p-4 my-2'>
									<p className='text-2xl font-semibold mb-2 mr-2'>Rp. {order.OrderDetail?.finalPrice?.toLocaleString('id-ID') || '0'}</p>
									<span
										className={`text-white text-[11px] px-3 py-1.5 rounded ${
											order.OrderDetail?.paymentStatus?.toLowerCase() === 'cancel' ? 'bg-red-600' : order.OrderDetail?.paymentStatus?.toLowerCase() === 'pending' ? 'bg-yellow-500' : 'bg-green-600'
										}`}
									>
										{order.OrderDetail?.paymentStatus ? order.OrderDetail.paymentStatus.charAt(0).toUpperCase() + order.OrderDetail.paymentStatus.slice(1).toLowerCase() : 'Tidak diketahui'}
									</span>

									<span className='bg-gray-600 text-white text-[11px] ml-2 px-2 py-1.5 rounded'>
										{order.OrderDetail?.PaymentMethod?.name || 'Metode tidak diketahui'} (
										{order.OrderDetail?.paymentDate
											? new Date(order.OrderDetail.paymentDate).toLocaleString('id-ID', {
													year: 'numeric',
													month: 'long',
													day: 'numeric',
											  })
                    ...
                    <IoIosArrowForward className="mx-1 font-bold" />
							...
							<div className='border dark:border-gray-500 rounded-lg p-4'>
								<div className='flex items-center gap-3'>
									...
									<div>
										<p className='text-md font-semibold'>{order.ShippingServices?.[0]?.shippingService || 'Belum tersedia'}</p>

										<p className='text-md text-gray-500'>Resi: {order.OrderDetail.receiptNumber || '-'}</p>
									</div>
								</div>
							</div>
						</div>
						...
						<div className='mt-6 ml-2'>
							<p className='font-semibold text-gray-500 text-sm'>Produk (total {order.OrderDetail?.OrderProducts?.length || 0} item)</p>
								...
								<ul className='mt-1 text-blue-700 font-semibold space-y-1'>
									{order.OrderDetail.OrderProducts.map((op) => (
										<li key={op.id}>
											...
											>
												{op.Product?.name || 'Produk tidak ditemukan'} ({op.productQty}x)
											</Link>
										</li>
									))}
								</ul>
              ...
              <button className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700">
                Tandai diterima
								...
								<div className='flex gap-2'>{/* <DropdownCancelOrder orderId={order.id} /> */}</div>
							</div>
						</div>
					) : (
