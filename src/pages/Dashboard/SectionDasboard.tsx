...
function SectionDasboard() {
	...
	useEffect(() => {
		...
		const functionGetinstallmentsOrders = async () => {
			...
			if (result.success && result.responseObject) {
				orderInstallment.current = result.responseObject as unknown as OrderItem[];
			}
		};
		...
		<div className='space-y-6 pl-3 lg:pl-3 '>
			<div className='px-4 py-3 bg-white rounded-xl shadow dark:bg-gray-800 dark:text-white'>
				<h3 className='my-2 text-md font-semibold text-gray-800 dark:text-white flex items-center'>
					<span className='mr-2'>👋</span> {getGreeting()} {JSON.parse(localStorage.getItem('user') || '{}').fullname || 'User'}
				</h3>
			...
			<div className='px-4 py-1 bg-white rounded-xl shadow dark:bg-gray-800 dark:text-white'>
				<div className='p-4 bg-red-50 dark:bg-red-800 rounded-lg mt-4'>
					<div className='flex justify-between'>
						<div className='mx-1'>
							<h3 className='text-theme-sm font-medium text-gray-800 dark:text-white'>Order Belum Lunas</h3>
							<span className='py-2 text-2xl font-bold text-red-400 dark:text-white'>{orderInstallment.current.length}</span>
						</div>
				...
				{orderInstallment.current.length == 0 && (
					<div className='flex items-center justify-center h-48'>
						<p className='text-theme-sm font-medium text-gray-500 dark:text-gray-300'>Tidak ada Order Yang Belum Lunas</p>
					</div>
				)}
				...
				{orderInstallment.current.map((order, index) => {
					...
					return (
						...
						>
							<div className='flex justify-between'>
								<div className='my-2'>
									<h4 className='text-theme-sm font-medium text-gray-800 dark:text-white'>{order.OrderDetail.code}</h4>
									<p className='text-theme-xs text-gray-500 dark:text-gray-300'>{order.OrdererCustomer.name}</p>
								</div>
								<span className='text-theme-xs text-gray-400 dark:text-gray-500 my-2'>{formatToReadableDate(order.createdAt)}</span>
							</div>
			...
			<div className='px-4 py-1 bg-white rounded-xl shadow dark:bg-gray-800 dark:text-white'>
				<div className='p-4 bg-yellow-50 dark:bg-yellow-800 rounded-lg mt-4'>
					<div className='flex justify-between'>
						<div className='mx-1'>
							<h3 className='text-theme-sm font-medium text-gray-800 dark:text-white'>Order Belum Diproses</h3>
							<span className='py-2 text-2xl font-bold text-yellow-500 dark:text-white'>{orderPending.current.length}</span>
						</div>
			...
			<div className='px-4 py-3 bg-white rounded-xl shadow dark:bg-gray-800 dark:text-white'>
				<h3 className='my-2 text-md font-semibold text-gray-800 dark:text-white'>Atur Profil Pengguna</h3>
				...
				<p className='text-theme-sm text-gray-500 dark:text-gray-300'>Anda dapat mengatur informasi profil pengguna, termasuk data pribadi, preferensi, dan pengaturan akun Anda.</p>
